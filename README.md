...
- **多站点巡检**：支持 20 个 Amazon 站点，每个站点独立配置 ASIN 列表
- **抓取字段**：售价、划线价、星级、评论数、卖家、库存、父体 ASIN、活动标、AC 标、Coupon、产品信息
- **产品信息**：原样抓取各站点 Product information 所有折叠区块（Features & Specs、Additional details 等），点击「查看」展开浮层查看完整键值对，支持 20 个站点多语言
- **参考对比**：导入 Excel 预设期望值，自动标红偏差项；导入时自动将 ASIN 按站点填入巡店面板
- **历史快照**：每次巡检后自动保存，支持与上次对比
- **智能重试**：一键重试失败项，自动降低并发
- **定时巡店**：Cron 表达式配置，到点自动触发，关闭主窗口后托盘继续运行；修改配置后无需手动触发即生效
- **站点管理**：内置 20 个 Amazon 站点，可启用/禁用，邮编统一管理；启用的站点实时同步到巡店面板下拉
...
| 标签 | 说明 |
|------|------|
| 巡店 | 站点分组卡片输入 ASIN，每个站点独立配置，开始巡店查看结果 |
| 参考数据 | 导入 Excel 预设期望值，显示上次导入信息，自动填充巡店面板 |
| 定时 | 配置 Cron 定时自动巡店，开关实时生效 |
| 日志 | 实时查看每条任务执行日志 |
| 历史 | 近 10 次巡店记录，点击查看详情 |
| 站点 | 管理 20 个 Amazon 站点的启用状态和配送邮编，已启用站点排在前面 |
| 设置 | 并发参数、钉钉、系统设置 |
...
- 缺货商品（Out of Stock）价格和划线价自动清空，避免误抓页面推荐区的价格
- 数据存储在本地 5 个 JSON 文件（settings.json、state.json、history.json、reference.json、sites.json），卸载应用不会自动删除
- 定时任务在电脑睡眠/休眠时无法触发，建议关闭自动睡眠；修改巡店配置后无需手动触发，定时任务自动读取最新配置
- 钉钉推送需同时满足：启用 Webhook + 已导入参考数据
- 参考数据 Excel 必须包含「ASIN」和「站点」两列（站点值如 CA、US、AU、MX），缺少「站点」列将报错
